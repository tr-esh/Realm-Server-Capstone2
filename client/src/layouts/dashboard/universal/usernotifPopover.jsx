...
const UserNotifPopover = () => {
    ...
    useEffect(() => {
        // Function to fetch data and update notifications
        const fetchData = async () => {
            try {
                // Fetch data from the WQI endpoint
                ...
                // Assuming the structure of latestWQIValuesAbove51 matches the notifications structure
                const newNotifications = latestWQIValuesAbove51.map(document => ({
                    id: document._id, // Assuming _id is present in your WQI documents
                    ...
                    createdAt: new Date(document.date), // Assuming date field is present in your WQI documents
                    isUnRead: !localStorage.getItem(`notification_${document._id}`), // Check if notification is read
                }));
    
                // Update notifications state with the new notifications
        ...
        // Fetch data on component mount and set interval for fetching data periodically
    ...
    const handleOpen = (event) => {
        ...
        setTotalUnRead(0); // Reset totalUnRead when opening the popover
    };
...
const renderContent = (notification) => {
    ...
    if (notification.type === 'station_approval') {
        return {
...
export default UserNotifPopover;
